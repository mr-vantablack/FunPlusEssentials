...
namespace FunPlusEssentials
{
    ...
    public class PlagueController : MonoBehaviour
    {
        ...
        public void Awake()
        {
            ...
            {
                roundDuration = float.Parse(PhotonNetwork.room.customProperties["RD004'"].ToString());
                CuteLogger.Meow("2");
        ...
        public IEnumerator RestartRound()
        {
            ...
            if (PhotonNetwork.player.isMasterClient)
            {
                Helper.Room.GetComponent<PhotonView>().RPC("StartRoundRPC", PhotonTargets.All, null);
            }
            yield break;
        }
        [FunRPC]
        public void StartRoundRPC()
        {
           ...
           // _midOfRound = false;
            ...
            roundDuration = PhotonNetwork.room.customProperties["RD004'"].Unbox<float>();
